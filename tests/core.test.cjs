...
test('HTML entrypoints exist and all application files parse',()=>{const dir=path.join(__dirname,'../dist'),html=fs.readFileSync(path.join(dir,'index.html'),'utf8');for(const m of html.matchAll(/(?:src|href)="([^"#]+)"/g)){if(!m[1].startsWith('http'))assert.ok(fs.existsSync(path.join(dir,m[1])),m[1]);}for(const file of ['app.js','core.js','deck.js'])new vm.Script(fs.readFileSync(path.join(dir,file),'utf8'));});
