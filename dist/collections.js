...
function catalog(seed){return {sets:[{id:'tcf-tache1',name:'TCF Tâche1',description:'350 条 B1–B2 日常法语表达',active:true,deleted:false}],cards:seed.map(c=>({...c,setId:'tcf-tache1',deleted:false}))};}
function validateCatalog(raw){
 ...
 const sets=raw.sets.map(s=>{if(!s||!validId(s.id)||ids.has(s.id)||!text(s.name,100,true)||!text(s.description,1000)||typeof s.active!=='boolean'||typeof s.deleted!=='boolean')throw Error('句集名称或编号无效');ids.add(s.id);return {id:s.id,name:s.name,description:s.description,active:s.active,deleted:s.deleted};});
...
const api={catalog,validateCatalog,initial,validate,activeCards,removeForever};if(typeof module!=='undefined'&&module.exports)module.exports=api;else root.ParoleCollections=api;
