...
function create({account=null,accountReady=()=>true,storage,getState,applyState,validate,onStatus=()=>{},onConflict=()=>{},fetchImpl=fetch}){
 ...
 async function request(method,body){
  ...
  try{
   const r=await fetchImpl(config.url+'/api/sync',{method,headers:{...(account?{'X-Parole-Account':account.id}:{Authorization:'Bearer '+config.key}),...(body?{'Content-Type':'application/json'}:{})},body:body?JSON.stringify(body):undefined,signal,credentials:account?'same-origin':'omit',redirect:'error',cache:'no-store',referrerPolicy:'no-referrer'});
   if(!r.ok){const e=Error(r.status===401?(account?'登录已失效或账户已切换，请刷新后登录。':'同步密钥不正确，请重新连接。'):r.status===409?'云端刚有更新，请再次点击同步。':r.status===413?'数据超过同步大小限制，请导出备份。':'连接失败，请检查后端配置、网络访问和网址。');e.status=r.status;throw e;}
