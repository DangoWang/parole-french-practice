...
function renderAI(){
 const box=$('ai-feedback');if(!box)return;
 ...
 if(aiResult){
  ...
  box.innerHTML='<h3>AI 建议 '+aiResult.score+' / 10 <small>GPT-4o mini</small></h3><p>'+esc(aiResult.explanation)+'</p><div class="diff ai-answer" lang="fr">'+marked+'</div>'+(aiResult.errors.length?'<ul>'+aiResult.errors.map(e=>'<li><span lang="fr">'+esc(e.quote||'缺少表达')+' → '+esc(e.replacement)+'</span><br>'+esc(e.reason)+'</li>').join('')+'</ul>':'<p>AI 未发现需要修改的地方。</p>')+'<div class="result-label">尽量保留原句的修改建议</div><div class="reference"><p lang="fr">'+esc(aiResult.corrected)+'</p></div><p class="muted">AI 判断可能有误，不是官方 TCF 分数。'+(scoreTouched?'已保留你的自评分。':'建议分数已填入，你可以调整。')+'</p>';
  return;
 }
...
async function runAI(){
 if(!current||!revealed||aiBusy||!$('answer').value.trim())return;
...
function setupAISettings(){
 ...
 $('ai-test').onclick=async()=>{
  ...
  try{const r=await AI.grade({key,card:{zh:'我建议你提前预订。',fr:'Je te conseille de réserver à l’avance.',group:'旅行'},answer:'Je te recommande de réserver à l’avance.',signal:controller.signal});if(!controller.signal.aborted)$('ai-settings-status').textContent='连接成功 · GPT-4o mini · 示例句评分 '+r.score+'/10。'+($('ai-key').value.trim()?'请点击保存 AI 设置以启用。':'');}
...
function updateStats(){document.querySelector('.nav-item[data-view="library"] small').textContent=state.catalog.sets.filter(s=>!s.deleted).length+' 集';renderHistory();const s=stats();$('today-date').textContent=new Intl.DateTimeFormat('zh-CN',{month:'long',day:'numeric',weekday:'long'}).format(new Date());$('streak').textContent=s.streak;$('today-count').textContent=s.today.size;$('daily-goal').textContent=state.settings.goal;$('due-count').textContent=s.due.length;$('due-chip').textContent=s.due.length;$('learned-count').textContent=s.learned;$('goal-progress').style.width=Math.min(100,s.today.size/state.settings.goal*100)+'%';$('progress-caption').textContent=(s.today.size>=state.settings.goal?(s.due.length?'新学目标已完成，请继续完成到期复习。':'新学目标已完成。'):`还需首次练习 ${state.settings.goal-s.today.size} 条新句。`)+` 今日已复习 ${s.reviewed.size} 条，不占新学名额。`;
...
function showCard(card,restore){resetAI();current=card;renderHistory();renderNotes();revealed=!!restore?.revealed;chosen=restore?.score||null;$('exercise').hidden=false;$('empty-state').hidden=true;$('card-kind').textContent=manual?'单条练习':state.progress[card.id]?'到期复习':'新表达';$('card-kind').className='tag'+(state.progress[card.id]?' review':'');$('card-group').textContent=(state.catalog.sets.find(s=>s.id===card.setId)?.name||'')+' · '+card.group+(card.star?' · 优先掌握':'');$('card-id').textContent=card.id;$('prompt').textContent=card.zh;$('answer').value=restore?.answer||'';$('answer').readOnly=revealed;$('answer').maxLength=2000;$('answer').placeholder='在这里写下你的法语表达…';$('word-count').textContent=($('answer').value.trim()?$('answer').value.trim().split(/\s+/).length:0)+' 词';$('answer-form').querySelector('.action-row').hidden=revealed;$('result').hidden=!revealed;$('result').innerHTML='';$('reveal').disabled=!$('answer').value.trim();if(revealed)renderResult();else if(view==='practice')setTimeout(()=>$('answer').focus({preventScroll:true}),30);}
function empty(){resetAI();current=null;renderHistory();renderNotes();$('exercise').hidden=true;$('empty-state').hidden=false;const st=stats(),list=allowed(),scheduled=deck.filter(c=>state.progress[c.id]).sort((a,b)=>state.progress[a.id].due-state.progress[b.id].due);const laterToday=scheduled.filter(c=>state.progress[c.id].due>Date.now()&&C.dayKey(state.progress[c.id].due)===C.dayKey());let title,body,button;
...
function rebuildCatalog(){
 ...
 $('scope').innerHTML='<option value="all">全部新句类别</option>'+groupNames.map(g=>'<option>'+esc(g)+'</option>').join('');$('scope').value=scope;
}
